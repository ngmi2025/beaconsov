      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="e.g., What's the best credit card for travel rewards?"
            />
          ...
          <div>
              ...
              {tags.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {tags.map((tag) => (
                    <span
                      key={tag}
                      className="inline-flex items-center gap-1.5 px-3 py-1 bg-violet-600/20 text-violet-400 rounded-lg text-sm"
                    >
                      #{tag}
                      <button
                        type="button"
                ...
                {suggestedTags
                  ...
                  .map((tag) => (
                    <button
                      ...
                      type="button"
                      onClick={() => addTag(tag)}
                      className="px-2.5 py-1 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-slate-300 rounded-lg text-xs transition-colors"
                    >
                      + {tag}
                    </button>
                  ))}
            ...
            <p className="mt-1.5 text-xs text-slate-500">
              Tags help you filter and organize queries
            </p>
          </div>
        </div>
        ...
        <div className="bg-violet-600/10 border border-violet-500/20 rounded-2xl p-5">
          ...
          <ul className="text-sm text-slate-400 space-y-1.5">
            <li>• Be specific: "Best credit card for travel" vs "Best credit card"</li>
            <li>• Ask for recommendations: "Which card do you recommend for..."</li>
            <li>• Include context: "...for frequent international travelers"</li>
            <li>• Use tags like "comparison" or "vs" for comparison queries</li>
          </ul>
        </div>
